...
if (isset($_POST['add_to_cart']) && isset($_SESSION['email'])) {
    ...
    $cart_query = "SELECT * FROM cart WHERE user_id = {$_SESSION['user_id']} AND product_id = $product_id";
    $cart_result = mysqli_query($conn, $cart_query);
    
    if (mysqli_num_rows($cart_result) > 0) {
        // Update quantity if product exists
        $update_query = "UPDATE cart SET quantity = quantity + $quantity WHERE user_id = {$_SESSION['user_id']} AND product_id = $product_id";
    ...
    header("Location: shop.php?added=$product_id");
    exit();
}
  ...
  <section id="all-products" class="section-gap">
    ...
    if ($result && mysqli_num_rows($result) > 0) {
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo '      <p class="card-text">' . substr(htmlspecialchars($row['productDesc']), 0, 100) . '...</p>';
        echo '      <div class="d-flex justify-content-between">';
        if ($row['stock'] > 0) {
          echo '        <a href="#" class="btn btn-pastel flex-grow-1 me-2">Add to Cart</a>';
        } else {
          echo '        <button class="btn btn-outline-secondary flex-grow-1 me-2" disabled>Out of Stock</button>';
        }
        echo '        <button class="btn btn-outline-pastel wishlist-btn" data-product-id="' . $row['productID'] . '">';
